...
class Mwb_Bookings_For_Woocommerce_Admin {
	...
	public function mbfw_product_data_tabs_html() {
		...
		<div id="mwb_booking_cost_data" class="panel woocommerce_options_panel show_if_mwb_booking">
			...
			<?php do_action( 'mwb_mbfw_booking_costs_meta_section_add_fields', get_the_ID() ); ?>
		</div>
	...
	public function mwb_mbfw_get_all_events_date() {
		...
		foreach ( $orders as $order ) {
			...
			foreach ( $items as $item ) {
				...
				$all_events[] = array(
					'title' => $item['name'],
					'start' => $item->get_meta( '_mwb_mbfw_booking_from_date', true ) . 'T' . $item->get_meta( '_mwb_mbfw_booking_from_time', true ),
					'end'   => $item->get_meta( '_mwb_mbfw_booking_to_date', true ) . 'T' . $item->get_meta( '_mwb_mbfw_booking_to_time', true ),
				);
			}
		}
