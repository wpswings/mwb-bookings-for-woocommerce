...
You can now simply book time slots utilizing the WordPress booking plugin, confirm/reject bookings, display reservations on calendar listings, enable booking cancellations, promote booking availability, regulate booking duration, and much more using the Bookings plugin's capabilities.
...
####1) Centralized View For Bookings 
Customers can view their bookings in a centralized manner. They can see the name of the booking product (which is clickable), the payment method they used, the option to add the booking to their Google Calendar, the booking status, and the cancellation option (if the merchant has enabled this functionality). Furthermore, customers can also select whether they want to view the bookings in a list or grid format. 

####2) Sync Global Calendar With Airbnb 
...
####3) Set Booking Limit  & Weekoffs for Global Calendar
Merchants can now set a limit per date and per order for bookings over the Global Calendar. They can also mark particular days off for the week. This helps them manage overbooking by capping the total number of bookings allowed in a given period.

####4) Global Calendar Booking 
...
All requested booking plugin-based orders made by the customers are to be listed in the WooCommerce Order Listing table. Admin managers can conveniently sort the booking out of the orders using the Bookings tag available there.
...
####12) Add your Bookings to Google Calendar
...
####13) Offered Services & Additional Costs
With the WooCommerce appointments plugin, admin managers can easily create booking services and additional costs using the backend settings, and also can easily display them over the checkout page as well as over the individual bookable resource page.
...
* [WordPress Booking Demo : Different Pricing On Different Dates](https://demo.wpswings.com/bookings-for-woocommerce-pro/product/car-wash/?utm_source=booking-org&utm_medium=referral&utm_campaign=wpswings-bookings-demo)
...
* [Elementor Page Builder](https://wordpress.org/plugins/elementor/) is compatible with bookings plugin. This compatibility lets you use Elementor’s capabilities to add the “global calendar” to different pages without copying shortcodes. 
...
* Our Bookings plugin is compatible with [Ultimate Gift Cards for WooCommerce](https://wordpress.org/plugins/woo-gift-cards-lite/) and [Gift Cards for WooCommerce Pro](https://wpswings.com/product/gift-cards-for-woocommerce-pro/). This enables merchants to run offers wherein they can offer gift cards for salons, spas, etc, along with their booking products. 



== WHAT PREMIUM VERSION OF BOOKINGS FOR WOOCOMMERCE PRO OFFERS ==

####1. Allow Single-Day Booking For Customers 

Utilize this functionality of the bookings plugin if merchants want their customers to book for a single day/date, not multiple days. This feature is suitable if you have a booking product that you want to enable for a day/date. 

**NOTE:** This functionality works for Single Calendar. 


####2. Color Codes to Display Availability:
...
####3. Export Booking Order by Booking Name
...
####5. Resource’s Daily Maximum Number of Bookings
...
Purpose: Submit user support queries
Data Sent: Name, email, message (user-provided form data)
When: On form submission via support page
...
**WP Swings Demos**
Purpose: Provides users with access to live product demo previews to explore plugin features and functionality.
Data Sent: Standard web request data such as IP address, browser type, and referring URL.
When: Data is sent only when the user voluntarily clicks on a demo link. No automatic requests are made by the plugin.

Service endpoints:
https://demo.wpswings.com/bookings-for-woocommerce-pro/
Terms of Service:
https://wpswings.com/terms-and-conditions/
Privacy Policy:
https://wpswings.com/privacy-policy/
...
**Google Maps Embed**
Purpose: Used to display booking location maps on the frontend product pages.
Data Sent: The booking location/address string is sent as a query parameter in the embedded map URL, along with standard browser request data.
When: Every time a booking product page with location map is loaded.
...
Purpose: Used to load font assets (if referenced by styles).
Data sent: Standard web request data for font files.
When: When fonts are loaded.
...
19. Set a limit for bookings for a particular date, order, and week off for Global Calendar
...
* New: Latest Compatibility with WP [7.0] and WC [10.8.1]
...
= 3.10.2 - Released on 8th January 2026 =

* New: Set limit for booking for particular date in global calendar.
* New: Latest compatibility With WP[6.9.0] and WC[10.4.3].
...
* Fix: Booking Cancel issue fixed
