...
<div id="mwb_booking_general_data" class="panel woocommerce_options_panel show_if_mwb_booking">
	...
	<div id="mwb_booking_unit" class="options_group">
		<p class="form-field">
			...
			<input type="number" name="mwb_booking_unit_input" id="mwb_booking_unit_input" value="<?php esc_attr_e($this->setting_fields['mwb_booking_unit_input']) ;?>" step="1" min="1" style="margin-right: 7px; width: 4em;">
			<select name="mwb_booking_unit_duration" id="mwb_booking_unit_duration" class="" style="width: auto; margin-right: 7px;">
				
				<?php foreach ($this->get_booking_unit_duration_options() as $key => $value) : ?>
					<option <?php selected( $key , $this->setting_fields['mwb_booking_unit_duration']) ?> value="<?php echo $key ?>" ><?php echo $value ?></option>
				<?php endforeach ; ?>
		...
		<?php
			woocommerce_wp_checkbox(
				array(
					...
					'value'       => $this->setting_fields['mwb_admin_confirmation_required'],
