...
class Makewebbetter_Onboarding_Helper {
	...
	public function enqueue_styles() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_style( 'makewebbetter-onboarding-style', MWB_WC_BK_BASEURL . 'admin/css/makewebbetter-onboarding-admin.css', array(), '3.0.0', 'all' );

			// Uncomment Only when your plugin doesn't uses the Select2.
			wp_enqueue_style( 'makewebbetter-onboarding-select2-style', MWB_WC_BK_BASEURL . 'admin/css/select2.min.css', array(), '3.0.0', 'all' );
		}
	}
	...
	public function enqueue_scripts() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_script( 'makewebbetter-onboarding-scripts', MWB_WC_BK_BASEURL . 'admin/js/makewebbetter-onboarding-admin.js', array( 'jquery' ), '3.0.0', true );
			...
			// Uncomment Only when your plugin doesn't uses the Select2.
			wp_enqueue_script( 'makewebbetter-onboarding-select2-script', MWB_WC_BK_BASEURL . 'admin/js/select2.min.js', array( 'jquery' ), '3.0.0', false );
		}
	}
	...
	public function add_onboarding_popup_screen() {
		...
		if ( $this->is_valid_page_screen() && $this->can_show_onboarding_popup() ) {
			require_once MWB_WC_BK_BASEPATH . 'extra-templates/makewebbetter-onboarding-template-display.php';
		}
	}
	...
	public function add_deactivation_popup_screen() {
		...
		if ( ! empty( $pagenow ) && 'plugins.php' === $pagenow ) {
			require_once MWB_WC_BK_BASEPATH . 'extra-templates/makewebbetter-deactivation-template-display.php';
		}
	}
	...
	public function is_valid_page_screen() {
		...
		if ( ! empty( $screen->id ) ) {
			...
			$is_valid = in_array( $screen->id, apply_filters( 'mwb_helper_valid_backend_screens', array() ), true ) && $this->add_mwb_additional_validation();
		}
	...
	public function add_on_boarding_form_fields() {
		...
		$fields = array(
			...
			rand() => array(                             // @codingStandardsIgnoreLine
				'id'          => 'monthly-revenue',
				'label'       => esc_html__( 'What is your monthly revenue?', 'mwb-bookings-for-woocommerce' ),
				'type'        => 'radio',
			...
			rand() => array(                               // @codingStandardsIgnoreLine
				'id'          => 'industry_type',
				'label'       => esc_html__( 'What industry defines your business?', 'mwb-bookings-for-woocommerce' ),
				'type'        => 'select',
				'name'        => 'industry_type_',
				'value'       => '',
				'multiple'    => 'yes',
			...
			case 'select2':
				...
				if ( ! empty( $options ) && is_array( $options ) ) {
					...
					<select class="on-boarding-select-field <?php echo esc_html( $select2 ); ?> <?php echo esc_html( $class ); ?>" id="<?php echo esc_html( $id ); ?>" name="<?php echo esc_html( $name ); ?>[]" <?php echo esc_html( $required ); ?> <?php echo esc_html( $is_multiple ); ?>>
						...
						<?php if ( 'select' == $type ) : ?>	
							<option class="on-boarding-options" value=""><?php esc_html( 'Select Any One Option...', 'mwb-bookings-for-woocommerce' ); ?></option>
						<?php endif; ?>
	...
	public function add_mwb_additional_validation( $result = true ) {
		...
		if ( ! empty( $_GET['tab'] ) && 'settings' !== $_GET['tab'] ) {     // @codingStandardsIgnoreLine
	 ...
	 * @param string $action_type The result of this validation.
	 * @return boolean
	 */
	protected function handle_form_submission_for_hubspot( $submission = false, $action_type = 'onboarding' ) {
		...
		if ( true == $result[ 'success' ] ) {                  // @codingStandardsIgnoreLine
			return true;
		} else {
			return false;
		}
	...
	private function hic_post( $endpoint, $post_params, $headers ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			$status_code = 500;
			$_response   = esc_html__( 'Unexpected Error Occured', 'mwb-bookings-for-woocommerce' );
			$errors      = $response;
	...
	protected function hubwoo_submit_form( $form_data = array(), $action_type = 'onboarding' ) {
		...
		if ( $response['status_code'] == 200 ) {                     // @codingStandardsIgnoreLine
