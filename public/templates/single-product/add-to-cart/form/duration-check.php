...
$unit_select   = ! empty( $product_meta['mwb_booking_unit_select'][0] ) ? sanitize_text_field( wp_unslash( $product_meta['mwb_booking_unit_select'][0] ) ) : 'customer';
...
$unit_duration = ! empty( $product_meta['mwb_booking_unit_duration'][0] ) ? sanitize_text_field( wp_unslash( $product_meta['mwb_booking_unit_duration'][0] ) ) : 'day';
...
$min_duration  = ! empty( $product_meta['mwb_booking_min_duration'][0] ) ? sanitize_text_field( wp_unslash( $product_meta['mwb_booking_min_duration'][0] ) ) : 1;
		...
		if ( empty( $range_picker ) || 'no' === $range_picker ) {
			...
			<div class="mwb-wc-bk-form-field" id="mwb-wc-bk-duration-div" >
				...
				<?php if ( ! empty( $min_duration ) && ( $min_duration !== $max_duration ) ) { ?>
					...
					?>
					<input id="mwb-wc-bk-duration-input" class="mwb-wc-bk-form-input mwb-wc-bk-form-input-number" type="number" name="duration" value="1" step="1" min="1" max="<?php echo esc_html( $max_duration ); ?>" required>
					<?php echo esc_html( sprintf( 'X %d %s', $unit_input, $unit_duration . ( ( $unit_input > 1 ) ? 's' : '' ) ) ); ?>
					<?php } ?>
			</div>
				<?php
